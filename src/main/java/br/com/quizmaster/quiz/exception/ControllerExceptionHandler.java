package br.com.quizmaster.quiz.exception;
    ...
    public ResponseEntity<StandardError> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpServletRequest request) {
        ...
        if (cause instanceof InvalidFormatException invalidFormatException) {
            if (invalidFormatException.getCause() instanceof DateTimeParseException) {
                ...
                StandardError err = new StandardError(System.currentTimeMillis(), HttpStatus.BAD_REQUEST.value(),"Erro de Formatação de Dados","Formato de data inválido. Use o padrão 'yyyy-MM-dd' ou 'yyyy-MM-ddThh:mm'.", request.getRequestURI());
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(err);
            }
        }
